...
export type UpdateGovernanceParams = {
    ...
    quorumStakeTon: bigint;
}

export function storeUpdateGovernanceParams(src: UpdateGovernanceParams) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeUint(4286453662, 32);
        b_0.storeUint(src.quorumPercent, 8);
        b_0.storeUint(src.timelockSeconds, 32);
        b_0.storeCoins(src.quorumStakeTon);
    };
}

export function loadUpdateGovernanceParams(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 4286453662) { throw Error('Invalid prefix'); }
    const _quorumPercent = sc_0.loadUintBig(8);
    const _timelockSeconds = sc_0.loadUintBig(32);
    const _quorumStakeTon = sc_0.loadCoins();
    return { $$type: 'UpdateGovernanceParams' as const, quorumPercent: _quorumPercent, timelockSeconds: _timelockSeconds, quorumStakeTon: _quorumStakeTon };
}
...
export function loadTupleUpdateGovernanceParams(source: TupleReader) {
    ...
    const _quorumStakeTon = source.readBigNumber();
    return { $$type: 'UpdateGovernanceParams' as const, quorumPercent: _quorumPercent, timelockSeconds: _timelockSeconds, quorumStakeTon: _quorumStakeTon };
}
...
export function storeTupleUpdateGovernanceParams(source: UpdateGovernanceParams) {
    ...
    builder.writeNumber(source.quorumStakeTon);
    return builder.build();
}
...
export type DAOVotingV2$Data = {
    $$type: 'DAOVotingV2$Data';
    owner: Address;
    gstdJetton: Address;
    quorumPercent: bigint;
    timelockSeconds: bigint;
    ...
    proposalAddresses: Dictionary<bigint, Address>;
}

export function storeDAOVotingV2$Data(src: DAOVotingV2$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        ...
        b_0.storeDict(src.proposalAddresses, Dictionary.Keys.BigInt(257), Dictionary.Values.Address());
    };
}

export function loadDAOVotingV2$Data(slice: Slice) {
    const sc_0 = slice;
    ...
    const _proposalAddresses = Dictionary.load(Dictionary.Keys.BigInt(257), Dictionary.Values.Address(), sc_0);
    return { $$type: 'DAOVotingV2$Data' as const, owner: _owner, gstdJetton: _gstdJetton, quorumPercent: _quorumPercent, timelockSeconds: _timelockSeconds, quorumStakeTon: _quorumStakeTon, totalStakedGSTD: _totalStakedGSTD, proposalCount: _proposalCount, executedCount: _executedCount, cancelledCount: _cancelledCount, proposalAddresses: _proposalAddresses };
}

export function loadTupleDAOVotingV2$Data(source: TupleReader) {
    ...
    const _proposalAddresses = Dictionary.loadDirect(Dictionary.Keys.BigInt(257), Dictionary.Values.Address(), source.readCellOpt());
    return { $$type: 'DAOVotingV2$Data' as const, owner: _owner, gstdJetton: _gstdJetton, quorumPercent: _quorumPercent, timelockSeconds: _timelockSeconds, quorumStakeTon: _quorumStakeTon, totalStakedGSTD: _totalStakedGSTD, proposalCount: _proposalCount, executedCount: _executedCount, cancelledCount: _cancelledCount, proposalAddresses: _proposalAddresses };
}

export function loadGetterTupleDAOVotingV2$Data(source: TupleReader) {
    const _owner = source.readAddress();
    const _gstdJetton = source.readAddress();
    const _quorumPercent = source.readBigNumber();
    const _timelockSeconds = source.readBigNumber();
...
export function storeTupleDAOVotingV2$Data(source: DAOVotingV2$Data) {
    const builder = new TupleBuilder();
    ...
    builder.writeCell(source.proposalAddresses.size > 0 ? beginCell().storeDictDirect(source.proposalAddresses, Dictionary.Keys.BigInt(257), Dictionary.Values.Address()).endCell() : null);
    return builder.build();
}

export function dictValueParserDAOVotingV2$Data(): DictionaryValue<DAOVotingV2$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeDAOVotingV2$Data(src)).endCell());
        },
        parse: (src) => {
            return loadDAOVotingV2$Data(src.loadRef().beginParse());
        }
    }
}
...
export function loadGovernanceStats(slice: Slice) {
    ...
    const _quorumStakeTon = sc_0.loadCoins();
    const _totalStakedGSTD = sc_0.loadCoins();
    return { $$type: 'GovernanceStats' as const, proposalCount: _proposalCount, executedCount: _executedCount, cancelledCount: _cancelledCount, quorumPercent: _quorumPercent, timelockSeconds: _timelockSeconds, quorumStakeTon: _quorumStakeTon, totalStakedGSTD: _totalStakedGSTD };
}
...
export function dictValueParserProposalData(): DictionaryValue<ProposalData> {
 ...
 type DAOVotingV2_init_args = {
    $$type: 'DAOVotingV2_init_args';
    owner: Address;
    gstdJetton: Address;
}

function initDAOVotingV2_init_args(src: DAOVotingV2_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.gstdJetton);
...
async function DAOVotingV2_init(owner: Address, gstdJetton: Address) {
    const __code = Cell.fromHex('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initDAOVotingV2_init_args({ $$type: 'DAOVotingV2_init_args', owner, gstdJetton })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

export const DAOVotingV2_errors = {
    ...
    48348: { message: "Only DAO can execute" },
    51706: { message: "Quorum not reached" },
    54682: { message: "Quorum stake must be positive" },
...
export const DAOVotingV2_errors_backward = {
    "Stack underflow": 2,
    "Stack overflow": 3,
    "Integer overflow": 4,
    ...
    "Only DAO can execute": 48348,
    "Quorum not reached": 51706,
    "Quorum stake must be positive": 54682,
...
const DAOVotingV2_types: ABIType[] = [
...
const DAOVotingV2_opcodes = {
    ...
    "UpdateGovernanceParams": 4286453662,
}

const DAOVotingV2_getters: ABIGetter[] = [
...
export class DAOVotingV2 implements Contract {
    
    public static readonly storageReserve = 0n;
    public static readonly errors = DAOVotingV2_errors_backward;
    public static readonly opcodes = DAOVotingV2_opcodes;
    
    static async init(owner: Address, gstdJetton: Address) {
        return await DAOVotingV2_init(owner, gstdJetton);
    }
    
    static async fromInit(owner: Address, gstdJetton: Address) {
        const __gen_init = await DAOVotingV2_init(owner, gstdJetton);
        const address = contractAddress(0, __gen_init);
        return new DAOVotingV2(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new DAOVotingV2(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  DAOVotingV2_types,
        getters: DAOVotingV2_getters,
        receivers: DAOVotingV2_receivers,
        errors: DAOVotingV2_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: CreateProposal | ExecuteProposal | "execution_confirmed" | UpdateGovernanceParams | "renounce" | Deploy) {
        ...
        if (message === "execution_confirmed") {
